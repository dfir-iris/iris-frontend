<!--
  ...
  row. Hovering the strip pops the full grouped list (Leads / Members /
  Case owners / Case access) with a search box. The popover stays open
  while the cursor is over it so the user can move from trigger to
  panel.
...
<script lang="ts">
	...
	// Hover-to-open with a tiny close-delay so the cursor can travel
	// from the trigger strip to the popover content without snapping it
	// shut mid-move. Mouse enter cancels any pending close; mouse leave
	// schedules one 120ms out. Click still toggles for keyboard /
	// touch users.
