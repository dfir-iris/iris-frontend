<script lang="ts">
	...
	let { open = $bindable(), onClose, event }: Props = $props();
	...
	let lastLoadedEventId = $state<number | null>(null);
	...
	const deleteComment = async (comment_id: number) => {
		if (event) {
			await CommentsService.remove('events', event.event_id, comment_id);
	...
	const refresh = async () => {
		...
		const commentsResponse = await CommentsService.list('events', event.event_id, {
			per_page: 10000
		});
		...
		lastLoadedEventId = event.event_id;
	...
	const saveComment = async () => {
		...
		if (editing_comment_id) {
			...
			await CommentsService.update('events', event.event_id, editing_comment_id, {
				...comment,
				comment_text
			});
		} else {
			await CommentsService.create('events', event.event_id, {
				comment_text: text
			});
		}
	...
	$effect(() => {
		const noteId = event?.event_id ?? null;
		...
		const noteChanged = noteId !== null && noteId !== lastLoadedEventId;
		...
		{#if event}
			<Dialog.Header class="border-b border-border/50 px-4 py-3">
				<Dialog.Title class="text-sm font-medium"
					>Comments on <span class="font-semibold">Event #{event.event_id}</span></Dialog.Title
				>
			</Dialog.Header>
