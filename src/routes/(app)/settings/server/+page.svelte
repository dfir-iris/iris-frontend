					...
					<div class="grid grid-cols-1 gap-3 p-4 text-xs sm:grid-cols-2">
						<label class="flex items-start gap-2 sm:col-span-2">
							<Switch
								...
								onCheckedChange={(v: boolean) => (form.chatbot_enabled = v)}
								disabled={saving}
							/>
							<div>
						...
						<div class="flex flex-col gap-1">
							...
							<Input
								...
								value={String(form.chatbot_model ?? '')}
								disabled={saving}
								oninput={(e) =>
									(form.chatbot_model =
										(e.currentTarget as HTMLInputElement).value || null)}
						...
						<div class="flex flex-col gap-1 sm:col-span-2">
							...
							<Input
								...
								autocomplete="off"
								class="h-7 text-xs"
								placeholder={form.chatbot_api_key_set
									? '•••••• (unchanged — leave blank to keep)'
									: 'sk-…'}
								value={String(form.chatbot_api_key ?? '')}
								disabled={saving}
								oninput={(e) =>
						...
						<div class="flex flex-col gap-1">
							...
							<Input
								...
								min="1"
								class="h-7 text-xs"
								value={String(form.chatbot_max_turns_per_conversation ?? '')}
								disabled={saving}
								oninput={(e) => {
									const raw = (e.currentTarget as HTMLInputElement).value;
									form.chatbot_max_turns_per_conversation =
										raw === '' ? 25 : Number(raw);
								}}
							/>
						</div>
						...
						<div class="flex flex-col gap-1">
							...
							<Input
								...
								min="1"
								class="h-7 text-xs"
								value={String(form.chatbot_max_tool_calls_per_turn ?? '')}
								disabled={saving}
								oninput={(e) => {
									const raw = (e.currentTarget as HTMLInputElement).value;
									form.chatbot_max_tool_calls_per_turn =
										raw === '' ? 8 : Number(raw);
								}}
							/>
						</div>
						...
						<div class="flex flex-col gap-1">
							...
							<Input
								...
								min="0"
								class="h-7 text-xs"
								value={String(form.chatbot_daily_token_budget_per_user ?? '')}
								disabled={saving}
								oninput={(e) => {
									const raw = (e.currentTarget as HTMLInputElement).value;
									form.chatbot_daily_token_budget_per_user =
						...
						<div class="flex flex-col gap-1">
							...
							<Input
								...
								min="0"
								class="h-7 text-xs"
								value={String(form.chatbot_daily_token_budget_org ?? '')}
								disabled={saving}
								oninput={(e) => {
									const raw = (e.currentTarget as HTMLInputElement).value;
									form.chatbot_daily_token_budget_org =
						...
						<label class="flex items-start gap-2 sm:col-span-2">
							<Switch
								checked={!!form.chatbot_auto_execute_read_tools}
								onCheckedChange={(v: boolean) =>
									(form.chatbot_auto_execute_read_tools = v)}
								disabled={saving}
							/>
							<div>
						...
						<label class="flex items-start gap-2 sm:col-span-2">
							<Switch
								checked={!!form.chatbot_redact_ips}
								onCheckedChange={(v: boolean) => (form.chatbot_redact_ips = v)}
								disabled={saving}
							/>
							<div>
						...
						<label class="flex items-start gap-2 sm:col-span-2">
							<Switch
								checked={!!form.chatbot_redact_emails}
								onCheckedChange={(v: boolean) =>
									(form.chatbot_redact_emails = v)}
								disabled={saving}
							/>
						...
						<label class="flex items-start gap-2 sm:col-span-2">
							<Switch
								checked={!!form.chatbot_redact_hashes}
								onCheckedChange={(v: boolean) =>
									(form.chatbot_redact_hashes = v)}
								disabled={saving}
							/>
							<div class="font-medium">
