...
<div class="hidden md:block">
	<Resizable.PaneGroup direction="horizontal" {onLayoutChange} class="items-stretch">
		<Resizable.Pane
			...
			{onCollapse}
			{onExpand}
			class="rounded-r-lg bg-muted/40"
		>
			<div class="mb-2 mt-4 px-4 text-lg font-semibold tracking-tight">
				<DropdownMenu>
			...
			<h2 class="mb-2 mt-4 px-4 text-lg font-semibold tracking-tight">Investigation</h2>
		...
		<Resizable.Pane defaultSize={defaultLayout[1]} minSize={10}>
			<div class="flex flex-col items-start gap-y-1 p-4">
				<div class="mb-4 flex w-full flex-row items-center">
					<div>
						<h1 class="ml-2 text-2xl font-semibold">{data.case_name}</h1>
						<p class="ml-2 text-sm text-muted-foreground">
							Opened on {new Date(data.open_date).toLocaleString()} by {data.owner?.user_name}
						</p>
					</div>
