...
<div class="fixed bottom-4 right-4 z-50">
	<Button variant="outline" size="sm" onclick={() => isOpen.set(true)}>Debug</Button>
</div>
...
<Drawer open={$isOpen} onOpenChange={(open) => isOpen.set(open)}>
	<DrawerContent side="bottom">
		<DrawerHeader>
			...
			<DrawerDescription>Tools to help debug the application</DrawerDescription>
		</DrawerHeader>
			...
			<div class="space-y-4">
				...
				{#if $showDebugInfo}
					<div class="space-y-4 rounded-md border p-4">
						<h3 class="text-sm font-medium">Client Information</h3>
						<pre class="max-h-40 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify(
								clientInfo,
								null,
								2
							)}</pre>

						<h3 class="text-sm font-medium">Server Information</h3>
						<pre class="max-h-40 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify(
								serverInfo,
								null,
								2
