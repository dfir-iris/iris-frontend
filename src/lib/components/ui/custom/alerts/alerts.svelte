...
<script lang="ts">
	...
	let { data }: { data: { alerts?: unknown[] } } = $props();
...
<div class="hidden md:block">
	...
	<Resizable.PaneGroup direction="horizontal" class="items-stretch">
		<Resizable.Pane defaultSize={30} collapsible minSize={7} maxSize={70}>
			{#if $isLoadingAlertsStore}
				<div class="p-4">
					<Skeleton class="mt-2 h-20 w-full bg-gray-100" />
					<Skeleton class="mt-2 h-20 w-full bg-gray-100" />
					<Skeleton class="mt-2 h-20 w-full bg-gray-100" />
					<Skeleton class="mt-2 h-20 w-full bg-gray-100" />
					<Skeleton class="mt-2 h-20 w-full bg-gray-100" />
