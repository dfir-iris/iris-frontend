<script lang="ts">
	...
	import { ShieldAlertIcon, ShieldCheckIcon, ShieldQuestionIcon } from 'lucide-svelte';
	...
	const compromiseStatusConfig = {
		...
		2: {
			...
			badgeClass:
				'bg-green-100 text-green-800 hover:bg-green-200 dark:bg-green-700/30 dark:text-green-400 dark:hover:bg-green-700/40'
		},
		3: {
			// Unknown
			label: 'Unknown',
			...
			badgeClass:
				'bg-gray-100 text-gray-800 hover:bg-gray-200 dark:bg-gray-700/30 dark:text-gray-400 dark:hover:bg-gray-700/40'
		}
	};

	$: currentStatusId = status_id ?? 3;
	$: config =
		compromiseStatusConfig[currentStatusId as keyof typeof compromiseStatusConfig] ||
		compromiseStatusConfig[3];
	...
	<Tooltip.Provider>
		<Tooltip.Root>
			<Tooltip.Trigger class="flex">
				...
				<Badge variant="outline" class="border-0 bg-transparent p-1 hover:bg-muted/50">
